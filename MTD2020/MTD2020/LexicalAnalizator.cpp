...
namespace LA
{
	void Lexicalanaliz(char inText[], Log::LOG, LEX::LexTable& lextable, ID::IdTable& idtable)
	{
		...
		for (int i = 0; i < strlen(inText); i++)
		{
			...
			if (provsep)
			{
				FST::FST fstint(buffer, FST_UWHOLE);
				if (FST::execute(fstint))
				{
					LEX::Entry lEntry = { LEX_UWHOLE , line, col };
				...
				FST::FST fstbool(buffer, FST_LOGIC);
				if (FST::execute(fstbool))
				{
					LEX::Entry lEntry = { LEX_LOGIC, line, col };
					strcpy(lEntry.buf, buffer);
					LEX::Add(*ltable, lEntry);
					dataType = ID::BOOL;
					idType = ID::V;
				...
				FST::FST fstret(buffer, FST_REPAY);
				if (FST::execute(fstret))
				{
					LEX::Entry lEntry = { LEX_REPAY, line, col };
					LEX::Add(*ltable, lEntry);
					idType = ID::V;
					goto link;
				}
				FST::FST fstprint(buffer, FST_MEOUT);
				if (FST::execute(fstprint))
				{
					LEX::Entry lEntry = { LEX_MEOUT, line, col };
					LEX::Add(*ltable, lEntry);
					idType = ID::V;
					goto link;
				}
				FST::FST fstprogram(buffer, FST_MASTER);
				...
				FST::FST fststrcpy(buffer, FST_STRDUP);
				...
				FST::FST fststrcat(buffer, FST_STRADD);
				...
				if (FST::execute(fstid))
				{
					...
					if (!isExecute)						//åñëè ïåðåìåííàÿ íå îáúÿâëåíà
					{
