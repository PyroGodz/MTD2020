...
namespace SA
{
	void ZeroDivision(LEX::LexTable& ltable, char* literal)		//проверка делени€ на ноль
	{
	...
	void CheckOct(LEX::LexTable& ltable, char* literal)		//проверка значени€ oct
	{
		for (unsigned int i = 0; i < (unsigned int)ltable.size; i++)
		{
			if (i == ltable.size - 1) {
				int k = 0;
				for (int t = 0; t < strlen(literal); t++)
				{
					if (literal[k] == '8' || literal[k] == '9')
					{
						throw ERROR_THROW(711, ltable.table[i].sn, ltable.table[i - 1].indxTI);
					}
					k++;
				}
			}
	...
	void OneProgram(LEX::LexTable& ltable, bool flag)		//проверка на одну главную функцию
	{
		int k = 0;
		if (flag)
		{
	...
	void BoolLt(LEX::LexTable& ltable, ID::IdTable& itable)		//проверка на тип данных в в условном операторе
	{
		for (unsigned int i = 0; i < (unsigned int)ltable.size; i++)
		{
	...
	void Operation(LEX::LexTable& ltable, ID::IdTable& itable) //проверка на применение знаков арифметических операций к идентификаторам или литералам типа string или bool
	{
		for (unsigned int i = 0; i < (unsigned int)ltable.size; i++)
		{
			char* buf[LT_MAXSIZE];
	...
	void Operationl(LEX::LexTable& ltable, ID::IdTable& itable) //проверка на наличие арифметических знаков
	{
		for (unsigned int i = 0; i < (unsigned int)ltable.size; i++)
		{
			char* buf[LT_MAXSIZE];
	...
	void Pereobyavl(LEX::LexTable& ltable, ID::IdTable& itable, char* buf, char* postfix, int line, int col) //проверка на повторное объ€вление переменной
	{
		for (unsigned int i = 0; i < (unsigned int)ltable.size; i++)
		{
	...
	void Proverka(LEX::LexTable& ltable, ID::IdTable& itable) //проверка на использование необъ€вленной переменной
	{
		for (unsigned int i = 0; i < (unsigned int)ltable.size; i++)
		{
			int k = 0;
			if (ltable.table[i].lexema != LEX_UWHOLE && ltable.table[i].lexema != LEX_FUNCTION && ltable.table[i + 1].lexema == LEX_ID)
			{
				for (unsigned int j = 0; j < (unsigned int)itable.size; j++)
				{
	...
	void RetValue(LEX::LexTable& ltable, ID::IdTable& itable)		//проверка типа данных возвращаемого значени€ функцией
	{
		for (unsigned int i = 0; i < (unsigned int)ltable.size; i++)
		{
			...
			if (ltable.table[i].lexema == LEX_FUNCTION && ltable.table[i + 1].lexema == LEX_ID)
			{
				...
				while (ltable.table[i].lexema != LEX_REPAY)
				{
					i++;
				}
	...
	void InicialId(LEX::LexTable& ltable, ID::IdTable& itable) ////проверка на совпадение типа данных справа и слева дл€ идентификаторов
	{
		ID::IDDATATYPE datatype;
		for (unsigned int i = 0; i < (unsigned int)ltable.size; i++)
		{
	...
	void InicialL(LEX::LexTable& ltable, ID::IdTable& itable) //проверка на совпадение типа данных справа и слева дл€ литералов
	{
		ID::IDDATATYPE datatype;
		for (unsigned int i = 0; i < (unsigned int)ltable.size; i++)
		{
	...
	void Parametrs(LEX::LexTable& ltable) //проверка на количество передаваемых параметров в функцию
	{
		int k = 0, m = 0, n = 0, z = 0;
		bool flag = false;
	...
	void TypeofParameters(LEX::LexTable& ltable, ID::IdTable& itable) //проверка типа данных передаваемых в функцию
	{
		for (unsigned int i = 0; i < (unsigned int)ltable.size; i++)
		{
	...
	void TypeofFuncParm(LEX::LexTable& ltable, ID::IdTable& itable) //проверка типов параматеров фунции
	{
		ID::IDDATATYPE data[LT_MAXSIZE];
